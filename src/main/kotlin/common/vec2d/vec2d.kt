...
class Vec2D <T : Number> private constructor(private val arithmetic: Arithmetic<T>,
                                             val x: T,
                                             val y: T) {
    // Pairwise operations
    operator fun plus(other: Vec2D<T>): Vec2D<T> =
        Vec2D(arithmetic, arithmetic.add(x, other.x), arithmetic.add(y, other.y))
    ...
    operator fun minus(other: Vec2D<T>): Vec2D<T> =
        Vec2D(arithmetic, arithmetic.subtract(x, other.x), arithmetic.subtract(y, other.y))

    operator fun times(other: Vec2D<T>): Vec2D<T> =
        Vec2D(arithmetic, arithmetic.multiply(x, y), arithmetic.multiply(other.x, other.y))

    operator fun div(other: Vec2D<T>): Vec2D<T> =
        Vec2D(arithmetic, arithmetic.divide(x, other.x), arithmetic.divide(y, other.y))
...
// Int -> Long
operator fun Vec2DInt.plus(other: Vec2DLong): Vec2DLong =
    Vec2D.long(this.x.toLong() + other.x, this.y.toLong() + other.y)
operator fun Vec2DInt.minus(other: Vec2DLong): Vec2DLong =
    Vec2D.long(this.x.toLong() - other.x, this.y.toLong() - other.y)
operator fun Vec2DInt.times(other: Vec2DLong): Vec2DLong =
    Vec2D.long(this.x.toLong() * other.x, this.y.toLong() * other.y)
